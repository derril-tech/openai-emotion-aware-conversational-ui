...
git clone https://github.com/derril-tech/openai-emotion-aware-conversational-ui-.git
cd openai-emotion-aware-conversational-ui
...
2. Execute the contents of `db/000-init.sql`
3. This creates the secure schema with RPC functions
...
|--------|----------|-------------|
| `POST` | `/agent/run` | Start emotion-aware conversation job |
| `GET` | `/jobs/{jobId}` | Get job status and result |
| `GET` | `/messages?conversationId=...` | Retrieve conversation history |
| `POST` | `/media/upload` | Upload audio/video media |
| `GET` | `/analytics?conversationId=...` | Get conversation analytics |
| `GET` | `/insights?conversationId=...` | Get emotional insights |
| `POST` | `/suggestions` | Generate AI-powered suggestions |
| `GET` | `/templates` | Get conversation templates |
| `GET` | `/export/markdown` | Export conversation as Markdown |
| `GET` | `/export/json` | Export conversation as JSON |
...
```bash
curl -X POST https://your-api.railway.app/agent/run \
...
```bash
# Test OpenAI connectivity and prompt quality
cd api
